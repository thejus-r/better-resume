...
const personalDetailsSchema = z.object({
  name: string().trim().min(1, { message: "Name should be atleast 1 character long!" }),
  role: string().trim().min(1, { message: "Role should be atleast 1 character long!" }),
  ...
  place: string().trim().min(1, { message: "Place should be atleast 1 character long!" })
})
